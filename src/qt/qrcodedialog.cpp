...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BLO")
    updateDisplayUnit();
}
...
{
    QString ret = QString("fellatio:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BLO unit input in lnReqAmount, we generate the URI with BLO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BLO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
