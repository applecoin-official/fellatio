...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uBLO
...
void ThreadScriptCheck() {
    RenameThread("fellatio-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1389685827, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x24936464 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x24, 0x93, 0x64, 0x64 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FellatioMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FellatioMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("FellatioMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fellatio-miner");
    ...
    try { loop {
        ...
        printf("Running FellatioMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FellatioMiner terminated\n");
        throw;
    }
}
